...
async function getSnowDays() {
  ...
  // Your code here
  const months = snowDays.map(row => row["Month"]);
  console.log(months);
  ...
  // Your code here
  const snowDays2023 = snowDays.map(row => row["Snow Days 2023-2024"]);
  ...
  // Your code here
   const snowDays2024 = snowDays.map(row => row["Month"] + " " + row["Snow Days 2024-2025"]);
